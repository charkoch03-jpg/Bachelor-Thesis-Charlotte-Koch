...
first_st = sio.loadmat(f"{ROOT}/firstFixationStartTimes.mat")["firstFixationStartTimes"]
first_en = sio.loadmat(f"{ROOT}/firstFixationEndTimes.mat")  ["firstFixationEndTimes"]
last_st  = sio.loadmat(f"{ROOT}/lastFixationStartTimes.mat") ["lastFixationStartTimes"]
last_en  = sio.loadmat(f"{ROOT}/lastFixationEndTimes.mat")   ["lastFixationEndTimes"]
tvec     = sio.loadmat(f"{ROOT}/timeVectorPic_clean.mat")["timeVector"] 
...
print("\n=== CUE TASK — FIRST picture dwell (relative to picture onset, ms) ===")
...
print("\n=== CUE TASK — LAST picture dwell (relative to picture onset, ms) ===")
print(f"Start: mean={sdesc['mean']:.2f} SD={sdesc['sd']:.2f} min={sdesc['min']:.2f} max={sdesc['max']:.2f} n={sdesc['n']}")
print(f"End:   mean={edesc['mean']:.2f} SD={edesc['sd']:.2f} min={edesc['min']:.2f} max={edesc['max']:.2f} n={edesc['n']}")
print(f"Dur:   mean={ddesc['mean']:.2f} SD={ddesc['sd']:.2f} min={ddesc['min']:.2f} max={ddesc['max']:.2f} n={ddesc['n']}")
