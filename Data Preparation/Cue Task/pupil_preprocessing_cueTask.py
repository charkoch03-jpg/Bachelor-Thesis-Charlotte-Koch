"""
Pupil preprocessing for Cue Task
...
PupilSizeRight = sio.loadmat("C:/Users/charl/OneDrive/Dokumente/Uni/7. Semester/Bachelor Thesis/ArrowTask/pupilSizeRightContinuos.mat")['pupilSizeRight']
PupilSizeLeft = sio.loadmat("C:/Users/charl/OneDrive/Dokumente/Uni/7. Semester/Bachelor Thesis/ArrowTask/pupilSizeLeftContinuos.mat")['pupilSizeLeft']

# === Flatten and combine for a quick global QC ===
...
# === Plot example raw signal (participant 1, left eye) ===
plt.figure(figsize=(12, 4))
plt.plot(PupilSizeLeft[0][0], label="Raw Left Eye (Participant 1)", alpha=0.7)
plt.title("Raw Left Eye Pupil Signal (Before Preprocessing) — Cue Task")
...
left_masked = mask_invalid(PupilSizeLeft, window=5)
right_masked = mask_invalid(PupilSizeRight, window=5)

# Quick visualization of masking effect
plt.figure(figsize=(12, 4))
plt.plot(PupilSizeLeft[0][0], label="Original", alpha=0.3)
plt.plot(left_masked[0], label="Masked invalids", alpha=0.8)
plt.title("Masking invalid values (Participant 1, Left Eye) — Cue Task")
plt.xlabel("Sample Index")
plt.ylabel("Pupil Size (a.u.)")
plt.legend()
plt.tight_layout()
...
# === Step 2: Remove extreme fluctuations (global derivative ±2 SD) ===
...
left_clean = remove_fluctuations(left_masked, z_thresh=2, window=5)
right_clean = remove_fluctuations(right_masked, z_thresh=2, window=5)

# Visualization of the full cleaning (up to this point)
plt.figure(figsize=(14, 4))
plt.plot(PupilSizeLeft[0][0], label="Original", alpha=0.3)
plt.plot(left_masked[0], label="Masked invalids", alpha=0.6)
plt.plot(left_clean[0], label="After fluctuation cleaning", alpha=0.9)
plt.title("Cleaning pipeline (Participant 1, Left Eye) — Cue Task")
plt.xlabel("Sample Index")
plt.ylabel("Pupil Size (a.u.)")
plt.legend()
plt.tight_layout()
...
# === Step 3: Linear interpolation of NaNs ===
...
# === Step 4: Median filter (kernel = 11; ensure odd and <= length) ===
def apply_median_filter(data, kernel=11):
    out = []
    for sig in data:
        k = min(kernel, len(sig) // 2 * 2 + 1)  # ensures odd kernel not larger than signal
        out.append(medfilt(sig, kernel_size=k))
    return out

left_final = apply_median_filter(left_interp, kernel=11)
right_final = apply_median_filter(right_interp, kernel=11)
...
plt.title("Participant 1, Block 1 (Cue Task) - Left Eye")
plt.xlabel("Sample Index")
...
# === Save cleaned data back to .mat files ===
sio.savemat(
    "data/interim/cue_task/pupilSizeLeft_cleanedInterpolated.mat",
    {"pupilSizeLeft": np.array([x.reshape(-1, 1) for x in left_final], dtype=object).reshape(-1, 1)},
)
sio.savemat(
    "data/interim/cue_task/pupilSizeRight_cleanedInterpolated.mat",
    {"pupilSizeRight": np.array([x.reshape(-1, 1) for x in right_final], dtype=object).reshape(-1, 1)},
)

print("\nPreprocessing complete. Cleaned data saved for Cue Task.")
